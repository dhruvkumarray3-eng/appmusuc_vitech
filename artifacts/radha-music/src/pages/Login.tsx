...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    setError("");
    const ok = await ownerLogin(telegramId.trim());
    if (!ok) setError("❌ Galat ID — sirf owner hi app unlock kar sakta hai.");
    setSubmitting(false);
  };
      ...
      <div className="z-10 w-full max-w-sm p-8 backdrop-blur-md bg-card/40 border border-border/50 rounded-2xl glow-box">
        ...
        {!loading && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={submitting || !telegramId.trim()}
            ...
            >
              {submitting ? <span className="flex items-center justify-center gap-2"><Loader2 size={14} className="animate-spin" />Checking...</span> : "🔓 App Unlock Karo"}
            </button>
